...
export const useFunctionsStore = defineStore('functions', () => {
  ...
  async function callFunction(name: string, payload: Record<string, any> = {}): Promise<any> {
    loading.value = true;
    error.value = null;
  ...
  async function uploadFunction(filename: string, content: string, notes?: string): Promise<any> {
    loading.value = true;
    error.value = null;
