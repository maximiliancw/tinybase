...
export const validationSchemas = {
  ...
  createSchedule: yup.object({
    ...
    unit: yup.string().when("method", {
      is: "interval",
      then: (schema) =>
        schema.required("Unit is required for interval schedules"),
    ...
    date: yup.string().when("method", {
      is: "once",
      then: (schema) =>
        schema.required("Date is required for one-time schedules"),
      otherwise: (schema) => schema.optional(),
    }),
    time: yup.string().when("method", {
      is: "once",
      then: (schema) =>
        schema.required("Time is required for one-time schedules"),
      otherwise: (schema) => schema.optional(),
    }),
    // Input data
    ...
    auth_portal_login_redirect_url: yup
      ...
      .when("auth_portal_enabled", {
        ...
        then: (schema) =>
          schema
            .required(
              "Login redirect URL is required when auth portal is enabled"
            )
            .url(
              "Please enter a valid absolute URL (e.g., https://app.example.com/dashboard)"
            )
            .test(
              "absolute-url",
            ...
            .test(
              "no-admin",
              "Login redirect URL must not point to /admin URLs",
              (value) => {
                if (!value) return false;
    ...
    auth_portal_register_redirect_url: yup
      ...
      .when("auth_portal_enabled", {
        ...
        then: (schema) =>
          schema
            .required(
              "Register redirect URL is required when auth portal is enabled"
            )
            .url(
              "Please enter a valid absolute URL (e.g., https://app.example.com/welcome)"
            )
            .test(
              "absolute-url",
            ...
            .test(
              "no-admin",
              "Register redirect URL must not point to /admin URLs",
              (value) => {
                if (!value) return false;
                return !value.includes("/admin");
              }
