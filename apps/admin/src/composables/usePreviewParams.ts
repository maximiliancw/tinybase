...
export function usePreviewParams() {
  ...
  const previewParams = computed(() => {
    ...
    if (route.query.preview === 'true') {
      params.preview = 'true';
      if (route.query.logo_url) params.logo_url = route.query.logo_url as string;
      if (route.query.primary_color) params.primary_color = route.query.primary_color as string;
      if (route.query.background_image_url)
        params.background_image_url = route.query.background_image_url as string;
  ...
  function withPreviewParams(to: string | { path: string; params?: Record<string, any> }) {
